    ...
    <div className="App">
      <header className="App-header">
        <form id='form' onSubmit={handleSubmit}>
          <input id='search' type="search" placeholder="Search a movie" value={searchTerm} onChange={handleChange} />
        </form>
      </header>
      <div id='movies-container'>
      {!isLoading && movies.length === 0 && <img src={NoResult} alt='no_result' />}

      {isLoading ? <img src={Loader} alt='loader'/> : movies.map((movie) => <Movie key={movie.id} {...movie} />)}
      </div>
    </div>
  );
}
